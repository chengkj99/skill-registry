...
`skill-registry` 扫描 `~/.claude/skills/` 与项目 `.claude/skills/` 等位置，为每条 skill 推断 `source`（官方 / 社区子类 / 本地 / 其他），并在终端树形、Markdown、JSON 中输出。

用户反馈：**位于全局 `~/.claude/skills/` 下的部分 skill，实际来自官方或社区插件，却被标为「本地」**（问题类型 B）。同时，默认树形清单展示字段偏少，不利于快速核对「这条到底是不是插件来的」。

## 2. 目标

1. **主目标**：降低全局目录下对插件来源 skill 的 **误判为「本地」** 的概率，使 `source` 与真实安装来源更一致。
...
- 将项目 `.claude/skills/` 下的 skill 从「本地」改为单独大类「项目」或与全局本地拆分展示（问题类型 A）。若后续需要，应另开规格说明。
...
1. **注册表查找键不完整**：`inferSource` 若仅按目录 basename 查 `pluginRegistry`，而注册表条目主要通过 **frontmatter 中的 `name`** 与目录名双键登记，则当 **目录名与 `name` 不一致** 且仅 `name` 在表中有记录时，会漏匹配而落入本地。
2. **软链路径未参与 cache 判定**：全局路径 `~/.claude/skills/foo` 的字符串中不含 `plugins/cache`，但实际 **`symlinkTarget` 指向 cache 内安装目录** 时，若不解析目标路径，会跳过 cache/manifest 分支，仅依赖 registry；registry 未命中时即本地。
...
2. 若 `skillPath` 位于项目 `projectSkillsDir` 下 → `本地`（现有逻辑保留，本轮不改语义）。
...
6. 其余兜底：全局下无命中 → `本地`；既非项目、非全局、非 cache 等已知形态 → `其他`（与现有实现一致处不随意改动）。
...
- 将某条 skill 从「本地」纠正为「官方」或「社区」后，仅改变 **展示与统计分组**；若与另一条同名 skill 的优先级比较发生变化，属于 **更符合真实安装关系** 的预期结果，需在测试中固定用例断言。
...
| 场景 | 期望 |
|------|------|
| 全局目录，registry 仅能通过 `frontmatter.name` 命中 | `source` 为官方或对应社区标签，非本地 |
| 全局目录，软链目标位于 cache 下且 manifest 为 Anthropic | `官方` |
| 全局目录，软链目标位于 cache 下且非官方 | `社区(<插件目录>)` |
| 无 `SKILL.md`、无 frontmatter、registry 无记录的真本地目录 | 仍为 `本地`（或与现有「其他」边界一致，不随意改变既有测试契约） |
...
- 若 README 中关于「本地」的说明与实现修正后的语义不一致，可在实现 PR 中 **小幅更新 README** 中与误判相关的说明句（不要求在本设计文档中预先改 README）。
